      ...
      <div className="container flex flex-wrap items-center justify-between mx-auto">
        ...
        <div className="flex md:order-2 ml-10 flex items-baseline space-x-4">
          ...
          {auth?.accounts && auth?.accounts.length > 0 ? (
            <Link href="/dashboard" className="text-white bg-rojo1  hover:bg-white hover:text-black font-medium rounded-lg text-sm px-3 py-2 text-center mr-3">
              {auth?.accounts[0]?.slice(0, 6)}...{auth?.accounts[0]?.slice(-4)}
            </Link>
          ) : (
            <button className="text-white bg-rojo1  hover:bg-white hover:text-black font-medium rounded-lg text-sm px-3 py-2 text-center mr-3 " onClick={connectWallet}>
              Connect Wallet
            </button>
          )}
        </div>
